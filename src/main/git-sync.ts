...
/** Host names declared in ~/.ssh/config (e.g. github-orion). */
  ...
  try {
    ...
    for (const line of raw.split(/\r?\n/)) {
      ...
      for (const h of m[1].trim().split(/\s+/)) {
        if (h && h !== '*' && !h.startsWith('!')) hosts.push(h)
      }
    }
...
/**
 * Ensure a Host alias exists in ~/.ssh/config pointing at github.com + IdentityFile.
 * Prefer names like github-orion (NOT github.com-orion — that fails DNS).
 */
export async function ensureSshHostAlias(opts: {
  alias: string
  keyPath: string
}): Promise<{ ok: boolean; created: boolean; message: string }> {
  let alias = opts.alias.trim()
  if (alias.startsWith('github.com-')) {
    alias = 'github-' + alias.slice('github.com-'.length)
  }
  if (!alias || /\s/.test(alias)) {
    return {
      ok: false,
      created: false,
      message: 'Alias inválido. Usa algo como github-orion (sin espacios).'
    }
  }
  const key = opts.keyPath.replace(/\\/g, '/')
  ...
  const hostRe = new RegExp(`^\\s*Host\\s+${escaped}\\s*$`, 'im')
  if (hostRe.test(raw)) {
    return { ok: true, created: false, message: `Host ${alias} ya existe en ~/.ssh/config` }
  }
  const block =
    '\n# KawaiiGPT — cuenta ' +
    alias +
    ...
    key +
    '\n  IdentitiesOnly yes\n'
  await fs.writeFile(cfgPath, (raw.trimEnd() ? raw.trimEnd() + '\n' : '') + block, 'utf8')
  return { ok: true, created: true, message: `Añadido Host ${alias} en ~/.ssh/config` }
}
...
export async function listSshKeys(): Promise<Array<{ name: string; path: string }>> {
  ...
  try {
    ...
    for (const n of names) {
      if (n.endsWith('.pub') || skip.has(n) || n.endsWith('.old')) continue
      if (!(n.startsWith('id_') || /github|orion|armando|ed25519|rsa/i.test(n))) {
...
export async function applyGitIdentity(identity: GitIdentity): Promise<GitSyncResult> {
  ...
  const key = identity.keyPath.split('\\').join('/')
  // Normalize alias: github.com-orion → github-orion (DNS fails on github.com-*)
  let hostAlias = (identity.hostAlias || '').trim()
  if (hostAlias.startsWith('github.com-')) {
    hostAlias = 'github-' + hostAlias.slice('github.com-'.length)
    steps.push(`Alias normalizado → ${hostAlias}`)
  }
  if (!hostAlias && identity.label) {
    const slug = identity.label.replace(/[^a-zA-Z0-9_-]/g, '').toLowerCase()
    if (slug) hostAlias = slug.startsWith('github-') ? slug : `github-${slug}`
    steps.push(`Alias por etiqueta → ${hostAlias}`)
  }
  if (hostAlias) {
    const ens = await ensureSshHostAlias({ alias: hostAlias, keyPath: identity.keyPath })
    steps.push(ens.message)
  }
  // Windows OpenSSH accepts forward slashes in -i
  ...
  if (status.remoteUrl) {
    const sshUrl = toGithubSshUrl(status.remoteUrl, hostAlias || undefined)
  ...
  try {
    ...
    await fs.writeFile(
      profilePath,
      JSON.stringify({ ...identity, hostAlias: hostAlias || identity.hostAlias, keyPath: identity.keyPath, updatedAt: Date.now() }, null, 2),
      'utf8'
    )
...
export async function testSshAuth(): Promise<GitSyncResult> {
  ...
  if (r.code !== 0) {
    ...
    if (/Permission denied \(publickey\)/i.test(err)) {
      error =
        'Permission denied (publickey). 1) En PowerShell: Start-Service ssh-agent; ssh-add ruta\a\tu_clave. 2) Alias en ~/.ssh/config debe coincidir con el remote (github-orion, no github.com-orion). 3) En el panel: elige la clave, pon el alias exacto del config y “Guardar identidad”.'
    } else if (/Could not resolve hostname/i.test(err)) {
      error =
        'No se resuelve el host del remote. El alias del remote (git@ALIAS:user/repo.git) debe existir como “Host ALIAS” en ~/.ssh/config (ej. Host github-orion).'
    } else {
      error = err || 'SSH/auth falló. Guarda identidad (clave + alias) y prueba de nuevo.'
    }
  }
  return {
    ok: r.code === 0,
