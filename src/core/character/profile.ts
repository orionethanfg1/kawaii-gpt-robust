...
export interface CharacterProfile {
  ...
  /**
   * Extra reference images (outfits, scenes, angles).
   * visualImageUrl remains the primary face/portrait for the UI bubble.
   */
  visualGallery?: Array<{
    id: string
    dataUrl: string
...
export function buildCharacterSystemPrompt(
  ...
  const galleryItems = (character.visualGallery || []).filter((g) => g.dataUrl)
  const galleryN = galleryItems.length
  const galleryLabels = galleryItems
    .map((g) => (g.label || '').trim())
    .filter(Boolean)
    .slice(0, 8)
  ...
  const identityBlock = [
    ...
    character.personality
      ? `Personalidad (OBLIGATORIA en cada respuesta): ${character.personality}`
      : '',
    character.style ? `Estilo de habla: ${character.style}` : '',
    traits ? `Rasgos: ${traits}` : '',
    `Responde siempre como ${character.name}, en personaje, nunca como asistente genérico.`,
    `Prohibido: «Estoy aquí para ayudarte», listar capacidades como FAQ, tono de soporte técnico salvo que pidan configurar la app.`
  ]
    .filter(Boolean)
    .join('\n')
  ...
  if (visualDesc) {
    ...
    galleryN > 0
      ? `Galería personal: ${galleryN} foto(s) TUYAS en Ajustes (escenas, ropa, ángulos)${
          galleryLabels.length ? ` · etiquetas: ${galleryLabels.join(', ')}` : ''
        }. SÍ las conoces: son tu referencia visual. Si preguntan por "tus fotos"/avatar, di cuántas hay y describe con la ficha canónica + variantes de ropa/escena; no digas que no las ves ni que solo puedes generar una nueva.`
      : '',
...
export function looksLikeAppearanceQuestion(text: string): boolean {
  return /\b(describ\w*|apariencia|c[oó]mo\s+te\s+ves|c[oó]mo\s+eres\s+f[ií]sic|aspecto\s+f[ií]sic|qu[eé]\s+tal\s+est[aá]s\s+de\s+look|tu\s+look|f[ií]sicamente|tus\s+fotos|tu\s+avatar|fotos\s+de\s+(tu|el)\s+avatar|fotos\s+de\s+\w+|galer[ií]a|has\s+visto\s+.*(foto|avatar)|mi\s+avatar|tu\s+imagen)\b/i.test(
    text
  )
}

/** True when the user asks about avatar/gallery images specifically. */
export function looksLikeAvatarGalleryQuestion(text: string): boolean {
  return /\b(fotos?\s+de\s+(tu|el|la)?\s*avatar|tu\s+avatar|tus\s+fotos|galer[ií]a|im[aá]genes\s+de\s+referencia|has\s+visto\s+.*(foto|avatar))\b/i.test(
    text
  )
}
